...
@interface MHShortVideoPublishManager () <
AliyunIExporterCallback, MHShortVideoUploadManagerDelegate>
...
static MHShortVideoPublishManager *_instance = nil;
static CGFloat exportRatio = 0.7; //合成暂用的时间比例

@implementation MHShortVideoPublishManager
#pragma mark - 单例

+ (instancetype)shared {
    if (_instance == nil) {
        
        _instance = [[MHShortVideoPublishManager alloc] init];
    }
...
- (BOOL)startUpload{
    ...
    [[MHShortVideoUploadManager shared] setManagerDelegate:self];
    [[MHShortVideoUploadManager shared] setCoverImagePath:coverPath
                                                   videoInfo:vInfo videoPath:self.mediaConfig.outputPath];
    [[MHShortVideoUploadManager shared]startUpload];
    return YES;
}

#pragma mark - AlivcShortVideoUploadManagerDelegate

- (void)uploadManager:(MHShortVideoUploadManager *)manager
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        switch (newStatus) {
            case AlivcUploadStatusCancel:
                weakSelf.newStatus = AlivcPublishStatusCancel;
                break;
            case AlivcUploadStatusFailure:
                weakSelf.newStatus  = AlivcPublishStatusFailure;
                break;
            case AlivcUploadStatusSuccess:
                weakSelf.newStatus  = AlivcPublishStatusSuccess;
                break;
            case AlivcUploadStatusUploading:
                weakSelf.newStatus  = AlivcPublishStatusPublishing;
                break;
