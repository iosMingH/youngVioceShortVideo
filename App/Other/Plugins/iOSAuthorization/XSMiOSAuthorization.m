    ...
    [[UNUserNotificationCenter currentNotificationCenter]requestAuthorizationWithOptions:UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert | UNAuthorizationOptionCarPlay completionHandler:^(BOOL granted, NSError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (granted) {
                ...
                [self createAlertWithMessage:[SettingLocalizableUtil localizedStringForKey:@"You disabled access, please go to Settings->Notifications  to set up access" withDefault:@"You have set up not allowing to send notifications. Please turn to stings->notifications and allow our app to receive notification."] isName:YES];
            }
        });
    }];
}
...
+(void)createAlertWithMessage:(NSString * )message isName:(BOOL)isName{
    ...
    NSString * GO = isName == YES?@"去设置":nil;
    UIAlertView * alert = [[UIAlertView alloc]initWithTitle:[SettingLocalizableUtil localizedStringForKey:@"XM_.FriendlyReminder_lu" withDefault:@"Friendly Reminder"] message:messages delegate:self cancelButtonTitle:@"取消" otherButtonTitles:GO , nil];
    [alert show];
}
...
//    NSString * urlString = @"App-Prefs:root=NOTIFICATIONS_ID";
