...
- (UICollectionView *)collectionView{
    if (_collectionView == nil) {
        ...
        UIView *bline = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-0.5, self.frame.size.width, 0.1)];
        bline.backgroundColor = ERGBColor(_param.botLineColor);
        UIView *tline = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 0.1)];
...
- (UIView *)lineView{
    if (_lineView == nil) {
        _lineView = [[UIView alloc] initWithFrame:CGRectMake(_param.margin_spacing+(self.itemWidth+_param.spacing)*_param.startIndex, self.collectionView.frame.size.height-7, self.itemWidth, 2)];
