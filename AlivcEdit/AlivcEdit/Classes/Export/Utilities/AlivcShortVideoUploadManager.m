...
- (void)dayeStartUploadImage{
    [AliyunSVideoApi dayeGetImageUploadAuthWithToken:[AliVideoClientUser shared].token title:@"DefaultTitle" filePath:self.coverImagePath tags:@"DefaultTags" handler:^(NSString * _Nullable uploadAddress, NSString * _Nullable uploadAuth, NSString * _Nullable imageURL, NSString * _Nullable imageId, NSError * _Nullable error) {
        if (error) {
            NSLog(@"error获取封面凭证:%@", error.description);
            [self handleUploadFailed];
            return;
...
- (void)dayeStartUploadVideo{
    _newStatus = AlivcUploadStatusUploading;
    __weak typeof(self)weakSelf = self;
    [AliyunSVideoApi dayeGetVideoUploadAuthWithWithToken:[AliVideoClientUser shared].token title:_videoInfo.title filePath:self.videoPath coverURL:self.coverImageUrl desc:_videoInfo.desc tags:_videoInfo.tags handler:^(NSString * _Nullable uploadAddress, NSString * _Nullable uploadAuth, NSString * _Nullable videoId, NSError * _Nullable error) {
        if (error) {
            NSLog(@"error:获取视频凭证%@", error.description);
            weakSelf.newStatus = AlivcUploadStatusFailure;
            [self handleUploadFailed];
